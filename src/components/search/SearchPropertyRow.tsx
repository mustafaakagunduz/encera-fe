// src/components/search/SearchPropertyRow.tsx
'use client';

import React, { useState } from 'react';
import Link from 'next/link';
import { useAppTranslation } from '@/hooks/useAppTranslation';
import { PropertySummary } from '@/services/smartSearchService';
import { ListingType, PropertyType } from '@/store/api/propertyApi';
import {
    MapPin,
    Home,
    Car,
...
interface SearchPropertyRowProps {
    property: PropertySummary;
    linkHref: string;
}

export const SearchPropertyRow: React.FC<SearchPropertyRowProps> = ({
    property,
    linkHref
}) => {
    ...
    const getListingTypeText = (type: string) => {
        switch (type) {
            case 'SALE':
                return isReady ? t('common.sale') : 'For Sale';
            case 'RENT':
                return isReady ? t('common.rent') : 'For Rent';
            default:
                return type;
        }
    };
    ...
    const getPropertyTypeText = (type: string) => {
        switch (type) {
            case 'RESIDENTIAL':
                return isReady ? t('common.residential') : 'Residential';
            case 'COMMERCIAL':
                return isReady ? t('common.commercial') : 'Commercial';
            case 'LAND':
                return isReady ? t('common.land') : 'Land';
            case 'DAILY_RENTAL':
                return isReady ? t('common.daily-rental') : 'Daily Rental';
            default:
                return type;
        }
    };
    ...
    const getRoomText = () => {
        ...
        if (property.roomCount && property.roomCount > 0) {
            return `${property.roomCount}+${property.hallCount || 0}`;
        }
        return null;
    };
                            ...
                            {property.pappSellable && (
                                <div className="absolute top-2 left-2">
                                    <div className="bg-orange-500 text-white px-2 py-1 rounded-full text-xs font-medium">
                                        {isReady ? t('search.encera-sellable') : 'ENCERA'}
                                    </div>
                                </div>
                            )}
                        </div>
